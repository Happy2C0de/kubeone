...
*/
#################################### Secgroups ###################################
resource "openstack_networking_secgroup_v2" "kube_apiserver" {
  name        = "${var.cluster_name}-apiserver"
  description = "Security group for the Kubeone Kubernetes cluster ${var.cluster_name} to access the apiserver"
}

resource "openstack_networking_secgroup_rule_v2" "kube_apiserver_6443" {
  ...
  security_group_id = openstack_networking_secgroup_v2.kube_apiserver.id
}

resource "openstack_networking_secgroup_rule_v2" "kube_apiserver_6443_ipv6" {
...
resource "openstack_networking_secgroup_rule_v2" "secgroup_ssh_ipv6" {
  count             = var.ipv6.enabled ? 1 : 0
  description       = "Allow SSH"
  direction         = "ingress"
  ethertype         = "IPv6"
...
resource "openstack_networking_secgroup_rule_v2" "secgroup_apiserver_ipv6" {
  count             = var.ipv6.enabled ? 1 : 0
  description       = "Allow kube-apiserver"
  direction         = "ingress"
  ethertype         = "IPv6"
